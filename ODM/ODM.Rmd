...
workflow_tex <- here("ODM/tables/workflow2.tex")

if (!file.exists(workflow_csv)){
  
  tbl_workflow <- data.frame(
  ...
  write_csv(tbl_workflow, "./tables/workflow.csv")
}

if (!file.exists(workflow_tex)){

  tbl_workflow <- read_csv(workflow_csv)

  knitr::kable(tbl_workflow, booktabs = T, format="latex") %>%
    kable_styling(full_width = F,  position = "right", font_size = 3) %>%
...
- strip `table` begin/end lines:
  ...
  ```
sciloop.pdf:
- Google Slides, File > Download > svg -> sciloop.svg
- on Mac terminal: 
  ```bash
